...
def _generate_uncertainty_canvas(
        ...
        decisions: Dict[int, InterventionState],  # Preserved: decisions parameter
        config: SystemConfig,
        action_line_y: int
) -> np.ndarray:
    """
    ...
    for plant_id, plant in active_plants.items():
        ...
        if state == InterventionState.IGNORE:
            ...
            box_color = tuple(config.classes[cls_id].color) if cls_id in config.classes else (0, 255, 0)
...
def draw_predictions(
    ...
    """
    # Initialize t_start to ensure it's always bound
    t_start = 0.0
